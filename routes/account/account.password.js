...
router.post('/change-password', requireAuth, async (req, res) => {
  ...
  if (!policy.test(newPassword)) {
    return res.status(422).json({ error: 'mot de passe faible', details: '12+, lettre, chiffre, spécial' });
  }

  const row = getPwd.get(uid);
  if (!row || !row.passwordHash) return res.status(404).json({ error: 'introuvable' });
